...
local function workflow_entries(config)
  ...
  add_mapping(entries, global.thread_rollback, "Roll back a thread to an earlier turn")
...
local function surface_entries(config, surface)
  ...
  elseif surface == "request" then
    ...
    add_mapping(entries, keymaps.respond, "Choose a response or answer")
    add_mapping(entries, keymaps.review, "Open the studied file-change review")
    add_mapping(entries, keymaps.accept, "Primary approve or grant action")
    add_mapping(entries, keymaps.accept_for_session, "Session-scoped approve or grant action")
    add_mapping(entries, keymaps.decline, "Decline or deny")
  ...
  elseif surface == "history_pager" then
    ...
    add_mapping(entries, "R", "Roll back the thread to the current turn")

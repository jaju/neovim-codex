...
assert(vim.fn.exists(":CodexChatOverlay") == 2, "CodexChatOverlay command should exist")
...
assert(chat_state.visible == true, "chat rail should be visible after opening")
assert(chat_state.mode == "rail", "chat command should open the side rail by default")
...
vim.wait(1000, function()
  return codex.get_chat_state().visible == true
end, 20)
assert(codex.get_chat_state().visible == true, "leaving the rail for an editor window should keep the rail visible")
...
vim.cmd("startinsert")
codex.open_chat_overlay()
local overlay_state = codex.get_chat_state()
assert(overlay_state.visible == true, "overlay command should open the centered overlay")
assert(overlay_state.mode == "reader", "overlay command should switch the shell into overlay mode")
assert(count_chat_shell_windows() == 3, "overlay mode should mount exactly one float shell")
assert(overlay_state.container_win ~= nil and vim.api.nvim_win_is_valid(overlay_state.container_win), "overlay mode should expose a container window")

codex.chat()
assert(codex.get_chat_state().visible == true, "chat command should switch back to the rail")
assert(codex.get_chat_state().mode == "rail", "chat command should reopen the side rail")
assert(count_chat_shell_windows() == 2, "returning to the rail should remove the overlay shell first")

vim.cmd("CodexChatOverlay")
assert(codex.get_chat_state().mode == "reader", "overlay command should switch the shell into centered mode")
assert(count_chat_shell_windows() == 3, "overlay mode should leave exactly one mounted shell")
vim.api.nvim_set_current_win(codex.get_chat_state().composer_win)
vim.api.nvim_feedkeys(termcodes("gR"), "xt", false)
vim.wait(1000, function()
  return codex.get_chat_state().mode == "rail"
end, 20)
assert(codex.get_chat_state().mode == "rail", "gR should switch the shell back to the rail")
assert(count_chat_shell_windows() == 2, "switching back to rail should not leave stale overlay windows behind")
